...
#   ./scripts/gcp/deploy-production.sh --version=v2.0.0
#   ./scripts/gcp/deploy-production.sh --version=v1-5-0 --build-dir=build
...
check_version_exists() {
    ...
    if gsutil ls "gs://${bucket}/releases/${version}/" >/dev/null 2>&1; then
        ...
        log_info "    ${CYAN}./deploy-production.sh --version=${version}-hotfix1${NC}"
        echo ""
        log_info "  Option 2: Update DEPLOY_VERSION in .env.production"
