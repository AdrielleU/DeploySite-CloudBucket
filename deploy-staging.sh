#!/usr/bin/env bash
# ============================================================================
# Deploy Static Frontend to Google Cloud Storage (Staging)
# ============================================================================
...
# Fall back to .env if .env.staging doesn't exist
...
# Staging-specific cache settings
ENV_CACHE_MAX_AGE=$(get_env_value "DEPLOY_CACHE_MAX_AGE" "$ENV_FILE" "86400")
ENV_HTML_CACHE_MAX_AGE=$(get_env_value "DEPLOY_HTML_CACHE_MAX_AGE" "$ENV_FILE" "1800")
...
if [ -z "$BUCKET_NAME" ]; then
    ...
    if [ -n "$PROJECT_ID" ]; then
        SUGGESTED_BUCKET="${PROJECT_ID}-staging"
    else
        SUGGESTED_BUCKET="my-app-staging"
    fi
...
log_info "${BLUE}============================================${NC}"
log_info "${BLUE}Frontend Deployment - STAGING${NC}"
log_info "${BLUE}============================================${NC}"
echo ""
